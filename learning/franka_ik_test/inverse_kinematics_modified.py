...
def qpos_from_site_pose(model, data,
  ...
  # 返回IKResult的命名元组
    An `IKResult` namedtuple with the following fields:
      # 关节位置增量
      qpos_delta: An (nq,) numpy array of joint positions.
  ...
  """
    ...
    # qpos_copy = copy.deepcopy(data.qpos)
    # 迭代搜索关节角度
    ...
    if target_quat is not None:
        ...
        print(update_nv)
    return IKResult(qpos_delta=update_nv, err_norm=err_norm)
