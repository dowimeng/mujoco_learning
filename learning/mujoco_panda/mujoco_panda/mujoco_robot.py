...
class MujocoRobot(object):
    ...
    def __init__(self, model_path=None, render=True, config=None, prestep_callables={}, poststep_callables={},
                 from_model=False):
        ...
        self._has_gripper = False  # by default, assume no gripper is attached 默认无夹爪
    ...
    def get_contact_info(self):
        # 没有很懂这个函数的意思  但是有可能之后有大用
        # 得到在机器人上物理接触的细节信息 位置 姿态 大小等
        """
    ...
    def site_pose(self, site_id, recompute=True):
        ...
        return self._sim.data.site_xpos[site_id].copy(), quaternion.as_float_array(
            quaternion.from_rotation_matrix(self._sim.data.site_xmat[site_id].copy().reshape(3, 3)))
