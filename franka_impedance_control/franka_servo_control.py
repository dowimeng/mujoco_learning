...
def impedance_controller(model, data):
    pass
    # 阻抗控制
    # data.ctrl = data.qfrc_bias.copy()
    # data.ctrl = data.qfrc_passive
    # print(data.qfrc_gravcomp)
...
z_ref = np.linspace(position_Q[2],position_Q[2],N)
x_all = []
y_all = []
z_all = []
...
while not glfw.window_should_close(window):
    ...
    while (time - time_prev < 1.0 / 60.0) and i < N:
        target_pos = np.array([x_ref[i], y_ref[i], z_ref[i]])
    ...
    x_all.append(data.site_xpos[0][0])
    y_all.append(data.site_xpos[0][1])
    z_all.append(data.site_xpos[0][2])
    i += 1
