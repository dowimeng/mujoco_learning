...
class ImpedanceFrankaGym(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.record_torque_impedance = None
    ...
    def impedance_controller(self,target_pos, target_quat,
        ...
        # self.data.ctrl = tau
        return tau, torque_impedance, IKResult
    ...
    def reset(self, seed=None, options=None):
        ...
        tau, torque_impedance, IKResult = self.impedance_controller(target_pos=self.target_pos[self.i], target_quat=self.target_quat,
        ...
        self.record_torque_real = self.data.qfrc_actuator
        self.record_torque_impedance = torque_impedance
    ...
    def step(self, action):
        ...
        # self.data.ctrl = IKResult.qpos
        self.data.ctrl = self.data.qfrc_gravcomp
        # self.data.ctrl = tau
        ...
        self.record_torque_real = np.vstack((self.record_torque_real, self.data.qfrc_actuator))
        self.record_torque_impedance = np.vstack((self.record_torque_impedance, torque_impedance))
        ...
        if self.i == self.N - 1:
            ...
            df = pd.DataFrame(self.record_torque_impedance)
            df.to_excel(writer, sheet_name='record_torque_impedance', index=False)
