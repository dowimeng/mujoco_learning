...
if __name__ == "__main__":
    ...
    df_record_data.to_csv('realtime_data_id.csv', mode='w', index= False, header=False)
    input_action = np.zeros(14)
    loop_time = time.time()

    f_joint = 2
    while (time.time() - loop_time < 12):
        _, _, _, _, record_data = custom_env.step(action=input_action)
        df_record_data = pd.DataFrame([record_data])
        df_record_data.to_csv('realtime_data_id.csv', mode='a', index=False, header=False)
        if (5 < time.time() - loop_time < 7):
            custom_env.data.qfrc_applied[f_joint] = 20
            # custom_env.data.xfrc_applied[0] = 100
        else:
            custom_env.data.qfrc_applied[f_joint] = 0
            # custom_env.data.xfrc_applied[0] = 0
