...
class ImpedanceFrankaGym(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # 观察空间和动作空间
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(39,), dtype=np.float64)
    ...
    def reset(self, seed=None, options=None):
        ...
        real_control = copy.copy(IKResult.qpos)
        real_control[1] = tau[1]
        self.data.ctrl = real_control
        ...
        self.record_torque_control = self.data.qfrc_actuator  # 控制力矩
    ...
    def step(self, action):
        ...
        self.weight = np.ones(7) * (1 * action[0] * action[0] + 2 * action[0] + 1)
        self.weight[1] = 1 * action[1] * action[1] + 2 * action[1] + 1
        # 这里修改spring和damp
        self.spring = np.ones(7) * 100
        self.damp = np.ones(7) * 10
        self.spring[1] = 100 * action[2] * action[2] + 200 * action[2] + 100
        self.damp[1] = 10 * action[3] * action[3] + 20 * action[3] + 10
        ...
        # 设计观察量 存储好各类变量
        self.record_torque_control = np.vstack((self.record_torque_control, self.data.qfrc_actuator))
        ...
        if self.i == self.N - 1:
            ...
            tau_yuzhi_i = np.absolute (tau[1] - self.data.qfrc_bias[1])
            if err_qpos_i > 0.1 and tau_yuzhi_i > 10:
                reward = 0
                # print("yes!")
            else:
                reward = 0
            print("record_err_x: ", np.mean(self.record_err_x))
            reward += ( 0.0499 - np.mean(self.record_err_x) ) * 5000
            print("reward: ", reward)
            done = True
        else:
            # 这部分是关节跟踪误差
