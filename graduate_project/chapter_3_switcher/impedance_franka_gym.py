...
class ImpedanceFrankaGym(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.origin_qpos = self.data.qpos
        self.data_copy = copy.copy(self.data)
    ...
    def step(self, action):
        ...
        self.data.ctrl = self.origin_qpos

        q_d_vel = self.target_traj_joint[self.i] - self.target_traj_joint[self.i - 1]
        tau_joint = self.data.qfrc_bias[self.joint] + \
              damp * (q_d_vel - self.data.qvel[self.joint]) + \
            spring * (self.target_traj_joint[self.i] - self.data.qpos[self.joint])

        self.data.ctrl[self.joint] = tau_joint
        ...
        if self.i == self.N - 1:
            # 计算平均误差
            error = self.record_qpos_real - self.record_qpos_control
            error = np.absolute(error)
            error = np.mean(error)

            # 峰值力和谷值力
            tor_max = np.max(self.record_torque_real)
            tor_min = np.min(self.record_torque_real)

            record = spring
            record = np.append(record,damp)
            record = np.append(record,tor_max)
            record = np.append(record,tor_min)
            record = np.append(record,error)

            df = pd.DataFrame(record)
            df.to_csv('record_single_impedance_2.csv', mode='a', index=False, header=False)
