...
xml_path = 'simple_pendulum.xml'  # xml file (assumes this is in the same folder as this file)
...
def controller(model, data):
    ...
    # position control; position/velocity control
    # set_position_servo(1, 100)
    # set_velocity_servo(2, 10)
    # data.ctrl[1] = np.pi

    # set_torque_servo(0,1)
    # data.ctrl[0] = 10*(np.pi-data.qpos[0]) # torque control like a spring
    # data.ctrl[0] = 100*(0.5-data.qvel[0])  # speed control
    # data.ctrl[0] = 100*(np.pi-data.qpos[0]) + 10+data.qvel[0]
...
cam.azimuth = 88.79999999999998;
cam.elevation = -30.199999999999925;
cam.distance = 7.60430518637421
cam.lookat = np.array([0.00877031664712993, -5.370270222087247e-19, 1.2977058010872078])
